 ...
 * @version 创建时间：2012-6-15 下午8:32:29 
 * 类说明 
 */
...
/**
 * The command line client to ZkMonitorCli.
 * 兼容所有Zookeeper原有的命令行功能
 */
public class ZkMonitorCli {
	private static final Logger LOG = Logger.getLogger(ZkMonitorCli.class);
	...
	private ZkMonitorClient zkMonitorClient;
	private MetricsConf metricsConf;
	private IGConf gConf;
	...
	static {
		...
		commandMap.put("addcam", "-s serverip -cip camip");
		commandMap.put("delcam", "-s serverip -cip camip");
		commandMap.put("delserver", "-s serverip");
		commandMap.put("getmetrics", "-s serverip ");
		commandMap.put("addcamOpt", "-cip camip");
		commandMap.put("lsMetric", "-s serverip");
		commandMap.put("lsBestServer", "");
		commandMap.put("lsAllMetrics", "");
	}
	...
	private class MyWatcher implements Watcher {
		public void process(WatchedEvent event) {
			if (getPrintWatches()) {
				ZkMonitorCli.printMessage("WATCHER::");
				ZkMonitorCli.printMessage(event.toString());
			}
		}
	}
	...
	public static void main(String args[]) throws KeeperException, IOException,
		...
		ZkMonitorCli main = new ZkMonitorCli(args);
		main.run();
	}

	public ZkMonitorCli(String args[]) throws IOException,
			InterruptedException {
		cl.parseOptions(args);
		System.out.println("Connecting to " + cl.getOption("server"));
		connectToZK(cl.getOption("server"));
		zkMonitorClient=new ZkMonitorClient();
		if(zk!=null){
			metricsConf=new MetricsConf(zk);
			gConf=new GConf(zk);
		}
	...
	public ZkMonitorCli(ZooKeeper zk) {
		this.zk = zk;
	}
	...
	void run() throws KeeperException, IOException, InterruptedException {
		if (cl.getCommand() == null) {
			System.out.println("Welcome to ZkMonitorCli!");
	 ...
	 */
	public static boolean createQuota(ZooKeeper zk, String path, long bytes,
			int numNodes) throws KeeperException, IOException,
			InterruptedException {
	...
	protected boolean processCmd(MyCommandOptions co) throws KeeperException,
	...
	protected boolean processZKCmd(MyCommandOptions co) throws KeeperException,
		...
		if (cmd.equals("quit")) {
			...
			if (args.length == 1) {
				System.out.println("printwatches is "
						+ (printWatches ? "on" : "off"));
			} else {
		...
		if (cmd.equals("create") && args.length >= 3) {
			...
			String val = args[2];
			path = args[3];
			System.err.println("Comment: the parts are " + "option " + option
					+ " val " + val + " path " + path);
			if ("-b".equals(option)) {
		...
		} else if(cmd.equals("addcamOpt")&&args.length>=3){   //添加摄像头到最优
			String camip=args[2];
			double metrics=0.0;
			Hashtable<String, Double> hashtable=metricsConf.getMetrisTable();
		    String bestServerIP=MetricsUtil.getBestServerXIP(hashtable);    
		    try {
		    	LOG.info("选取的最佳Server是"+bestServerIP+"转移前权值("+metricsConf.getMetricsByIP(bestServerIP)+")");
				boolean ret=zkMonitorClient.addCamera(bestServerIP, camip);
				LOG.info("转移后的权值是"+metricsConf.getMetricsByIP(bestServerIP));
				LOG.info("将"+camip+"摄像头转移到"+bestServerIP);
			} catch (TException e) {
				e.printStackTrace();
			}
			System.out.println("This is test " +metrics);
		}else if(cmd.equals("addcam")&&args.length>=5){   //添加摄像头
			String serverip=args[2];
			String camip=args[4];
			boolean ret;
			try {
				ret=zkMonitorClient.addCamera(serverip, camip);
				System.out.println("Add Camera "+camip +" to "+"Server "+serverip);
			} catch (TException e) {
				System.err.println("Add Camera failed");
			}
		}else if(cmd.equals("lsMetric")&&args.length>=3){  //获取某个采集服务器当前的状态
			String serverip=args[2];
			System.out.println(serverip);
			try {
				double metric =zkMonitorClient.getMetrics(serverip);
				System.out.println(serverip+" now metric = "+metric);
			} catch (TException e) {
//				System.err.println("lsMetric error :"+e.getMessage());
				e.printStackTrace();
			}
		}else if(cmd.equals("lsAllMetrics")&&args.length==1){ //获取所有服务器的状态列表
			try {
				Hashtable<String, Double> metrictable=(Hashtable<String, Double>) zkMonitorClient.getAllMetrics();
			    for(Entry<String, Double> entry:metrictable.entrySet()){
			    	System.out.println("["+entry.getKey()+" metrics ="+entry.getValue()+"]");
			    }
			} catch (TException e) {
				System.err.println("lsAllMetrics error :"+e.getMessage());
			}
			
		}else if(cmd.equals("lsBestServer")&&args.length==1){ //查找最好的服务器和状态
			  try {
				Map<String, Double> bestmap = zkMonitorClient.getBestMetEntry();  //其实这个map只有一个值
				 for(Entry<String, Double> entry:bestmap.entrySet()){
				    	System.out.println("[ Best Server is "+entry.getKey()+" metrics ="+entry.getValue()+"]");
				    }
			} catch (TException e) {
				System.err.println("lsBestServer error "+e.getMessage());
			}
		}else {
			usage();
		}
